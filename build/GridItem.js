...
var GridItem = function (_React$Component) {
  ...
  GridItem.prototype.calcPosition = function calcPosition(x, y, w, h, state) {
    ...
    var out = {
      ...
      width: w === Infinity ? w : Math.round(colWidth * wh + Math.max(0, wh - 1) * margin[0]),
      height: h === Infinity ? h : Math.round(rowHeight * wh + Math.max(0, wh - 1) * margin[1])
    };
  ...
  GridItem.prototype.mixinResizable = function mixinResizable(child, position) {
    ...
    var maxHeight = this.calcPosition(0, 0, cols - y, 0).height;
    ...
    var mW = Math.min(maxes.width, maxWidth)
    var mH = Math.min(maxes.height, maxHeight)
    //присваиваем минимальное maxWidth maxHeight или ширины ширине тили высоте
    //ограничиваем ресайз за границу мозаики
    var param = mW >= mH ? mH : mW
    var maxConstraints = [param, param];
    ...
    return _react2.default.createElement(
      _reactResizable.Resizable,
      {
        ...
        onResize: this.onResizeHandler("onResize"), lockAspectRatio: true
      },
      child
    );
  };
